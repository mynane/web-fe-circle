...
(function () {
    ...
    ui.Pagination = core.inherits(
        ...
        function (el, options) {
            ...
            if (Options.last) {
                plEnd += 1;
                items.push('<li class="' + pageItem + '" ui-page="last">' + Options.last + '</li>');
            }
            ...
            root.bothend = [];
            if (Options.first) {
                root.bothend.push(list[0]);
            }
            if (Options.last) {
                root.bothend.push(list[list.length - 1]);
                if (!Options.first) {
                    root.bothend.unshift(null);
                }
            }
            ...
            if (Options.info) {
                ...
                if (Options.items) {
                    pi.innerHTML = '共<span id="paginationCount">' + Options.items + '</span>条';
                } else {
                    pi.innerHTML = '共<span id="paginationCount">' + pages + '</span>页';
                }
            ...
            go: function (page) {
                ...
                if (pages <= max) {
                    ...
                    if (showPages === 'center') {
                        hide(ellipsis[0], '');
                        hide(ellipsis[1], '');
                        // setPageNumbers(page - Math.floor(max / 2 / 2) - ((pages - max) % 2 === 0 ? 0 : 1));
                        // setPageNumbers(page - Math.floor(max / 2 / 2) - ((pages%2 === max%2) ? 0 : 1));
                        // setPageNumbers(page - (Math.ceil((max-2)/2) - 1));
                ...
                if (page === 1) {
                    ...
                    if (first) {
                        dom.addClass(first, pageDisable);
                    }
                } else {
                    dom.removeClass(prev, pageDisable);
                    if (first) {
                        dom.removeClass(first, pageDisable);
                    }
                }
                if (page === pages) {
                    ...
                    if (last) {
                        dom.addClass(last, pageDisable);
                    }
                } else {
                    dom.removeClass(next, pageDisable);
                    if (last) {
                        dom.removeClass(last, pageDisable);
                    }
                }
